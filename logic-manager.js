...
export function testOcrRuleFromModal() {
    ...
    const nameInput = modalContainer.querySelector('#ocr-rule-name');
    const testerInput = modalContainer.querySelector('#ocr-tester-input');
    const resultContainer = modalContainer.querySelector('#ocr-tester-result');

    if (!patternInput || !testerInput || !resultContainer) return;
    ...
    const associatedIdSelect = modalContainer.querySelector('[name="associatedId"]');
    if (associatedIdSelect) rule.associatedId = associatedIdSelect.value;

    const result = testSingleOcrRule(testerInput.value, rule);

    if (result.success) {
        resultContainer.innerHTML = `<div class="tester-result-success"><i class="fa-solid fa-check-circle"></i> <strong>Sucesso!</strong> Valor extraído: <code>${result.value}</code></div>`;
    } else {
        resultContainer.innerHTML = `<div class="tester-result-error"><i class="fa-solid fa-times-circle"></i> <strong>Falhou:</strong> ${result.message}</div>`;
    }
}
...
export async function adjustAccountBalance(accountId) {
        ...
        <form id="adjust-balance-form">
            ...
            <div class="modal-body">
                <p style="color: var(--text-secondary); margin-bottom: 1rem;">
                    Defina o saldo correto atual para <strong>${account.name}</strong>.
                </p>
                <div class="form-group">
                    <label>Novo Saldo</label>
                    <div class="input-group-currency">
                        <span class="currency-symbol">R$</span>
                        <input type="number" step="0.01" id="new-balance-input" name="newBalance"
                            value="${account.balance || 0}" required autofocus>
                    </div>
                </div>
            </div>
    ...
    document.getElementById('adjust-balance-form').onsubmit = async (e) => {
        e.preventDefault();
        const newBalance = parseFloat(document.getElementById('new-balance-input').value.replace(',', '.'));
        ...
        try {
            ...
            showToast('Erro ao ajustar o saldo.', 'error');
        } finally {
            modalContainer.classList.remove('visible');
            setTimeout(() => { modalContainer.innerHTML = ''; }, 300);
        }
