...
export async function saveTransaction(form, isEdit = false) {
    ...
    try {
        ...
        const returnedMonthYear = data.monthYear || '';
        return { success: true, isEdit, shouldCheckForMonthAdvance: !isEdit, monthYear: returnedMonthYear };
    } catch (error) {
...
export async function parcelarCompra(form) {
    const formData = new FormData(form);
    ...
    try {
        if (!transactionId) throw new Error('Transação original não encontrada.');
        if (isNaN(installments) || installments < 2) throw new Error('Número de parcelas deve ser pelo menos 2.');
        if (isNaN(totalValue) || totalValue <= 0) throw new Error('Valor total inválido.');

        const original = appState.allTransactions.find(t => t.id === transactionId);
        if (!original) throw new Error('Transação original não encontrada no estado.');

        const batch = db.batch();
        const groupId = db.collection('financeiro_lancamentos').doc().id;
        const valuePerInstallment = totalValue / installments;
        const baseDescription = original.description.replace(/ \[\d+\/\d+\]$/, '');
        const baseDate = getDateObject(original.date);
        ...
        for (let i = 0; i < installments; i++) {
            ...
            const instData = {
                ...
                date: firebase.firestore.Timestamp.fromDate(instDate),
                monthYear: `${(instDate.getMonth() + 1).toString().padStart(2, '0')}-${instDate.getFullYear()}`,
        ...
        // Remove a transação original (compra avulsa)
        batch.delete(db.collection('financeiro_lancamentos').doc(transactionId));

        await batch.commit();
        showToast('Compra parcelada com sucesso!', 'success');
        return { success: true };
    } catch (error) {
        console.error('Erro ao parcelar compra:', error);
        showToast(error.message || 'Erro ao parcelar compra.', 'error');
        return { success: false };
    } finally {
        submitButton.disabled = false;
        submitButton.innerHTML = 'Salvar Parcelamento';
    }
}
